...
public:
    ShapeShader() {
        mProgram = new ShaderProgram("shape.vert", "shape.frag");
        mProgram->Compile();
    }
    ~ShapeShader() override {
        delete mProgram;
    }
...
protected:
    void OnCreate() override {
        ...
        auto playerBody = Physics::CreateCharacter(glm::vec3(0.0f, 1.0f, 0.0f), 0.25f, 1.5f);
        playerBody->SetPosition(glm::vec3(0.0f, 10.0f, 0.0f));
        mPlayer->AddComponent(CharacterMovement::Create(playerBody));
        ...
        mFloor = MeshBuilder()
                .SetBox(glm::vec3(100.0f, 0.1f,  100.0f))
                .SetTextureCoordScale(100.0f)
                .SetMaterial(MaterialBuilder()
                    ...
                    .Build())
                .Build();
        auto floorBody = Physics::CreateBodyBox(glm::vec3(100.0f, 0.1f, 100.0f), 0.0f);
    ...
    void OnTick(float dt) override {
        ...
        Gizmos::Begin(mScene->GetActiveCamera().lock().get(), mShapeShader);
        Physics::DrawDebug();
        Gizmos::End();
    }
    ...
    ShapeShader *mShapeShader{nullptr};
